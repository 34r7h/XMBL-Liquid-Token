...
describe("XMBLVault", function () {
  ...
  describe("Token Deposits and NFT Minting", function () {
    it("Should accept ETH deposit and mint XMBL NFT with TBA", async function () {
      ...
      await expect(tx)
        ...
        .withArgs(user1.address, ZeroAddress, depositAmount, 1, await vault.getTokenBoundAccount(1));
      ...
      expect(userNFTs).to.deep.equal([1n]);
    });
    ...
    it("Should accept ERC-20 token deposit and create NFT with TBA", async function () {
      ...
      await expect(tx)
        .to.emit(vault, "Deposit")
        .withArgs(user1.address, await usdc.getAddress(), depositAmount, 1, await vault.getTokenBoundAccount(1));
    ...
    it("Should calculate correct XMBL value based on bonding curve", async function () {
      ...
      const expectedValue = await vault.calculateXMBLValue(depositAmount, ZeroAddress);
    ...
    it("Should handle multiple deposits from same user", async function () {
      ...
      // Second deposit
      await vault.connect(user1).deposit(ZeroAddress, 0, { value: parseEther("0.5") });
    ...
    it("Should reject invalid deposit parameters", async function () {
      ...
      await expect(
        vault.connect(user1).deposit("0x1234567890123456789012345678901234567890", parseUnits("100", 6))
  ...
  describe("Token Bound Account Operations", function () {
    ...
    it("Should allow NFT owner to execute transactions from TBA", async function () {
      ...
      await vault.connect(user1).executeTBATransaction(1, await usdc.getAddress(), 0, transferData);
      
      expect(await usdc.balanceOf(tbaAddress)).to.equal(parseUnits("50", 6));
      expect(await usdc.balanceOf(user2.address)).to.equal(parseUnits("50", 6));
    });
  ...
  describe("Yield Distribution", function () {
    it("Should distribute yields proportionally to NFT deposit values", async function () {
      ...
      expect(await vault.accruedYields(1)).to.equal(parseEther("0.2"));
      expect(await vault.accruedYields(2)).to.equal(parseEther("0.1"));
    });
    ...
    it("Should allow users to claim individual NFT yields", async function () {
      ...
      expect(finalBalance.add(gasUsed)).to.be.closeTo(initialBalance.add(totalYield), parseEther("0.001"));
  ...
  describe("NFT Withdrawal", function () {
    it("Should allow NFT holder to withdraw underlying assets", async function () {
      ...
      const finalBalance = await user1.getBalance();
      expect(finalBalance.add(gasUsed)).to.be.closeTo(initialBalance.add(depositAmount), parseEther("0.001"));
    ...
    it("Should handle withdrawal when TBA has additional assets", async function () {
      ...
      expect(await usdc.balanceOf(user1.address)).to.equal(parseUnits("10100", 6)); // Original + TBA balance
    });
  });
  ...
  describe("Gas Optimization", function () {
    it("Should batch operations efficiently", async function () {
      ...
      for (let i = 0; i < 5; i++) {
        deposits.push(
          vault.connect(user1).deposit(ZeroAddress, 0, { value: parseEther("0.1") })
        );
      }
  ...
  describe("Integration Tests", function () {
    ...
    it("Should handle multiple users with different strategies", async function () {
      ...
      await vault.connect(user1).deposit(ZeroAddress, 0, { value: parseEther("2.0") });
      await vault.connect(user2).deposit(ZeroAddress, 0, { value: parseEther("1.0") });
      await vault.connect(user3).deposit(ZeroAddress, 0, { value: parseEther("0.5") });
  ...
  describe("Security Tests", function () {
    ...
    it("Should validate all inputs properly", async function () {
      ...
      // Invalid addresses
      await expect(
        vault.connect(user1).deposit("0x0000000000000000000000000000000000000000", parseEther("1.0"))
      ).to.be.revertedWith("Invalid token address");
      
      // Overflow protection
      await expect(
        vault.connect(user1).deposit(ZeroAddress, 0, { value: MaxUint256 })
      ).to.be.reverted;
    });
    ...
    it("Should handle edge cases gracefully", async function () {
      ...
      // Minimum deposit
      await vault.connect(user1).deposit(ZeroAddress, 0, { value: 1 });
