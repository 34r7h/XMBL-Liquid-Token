...
describe("XMBLToken", function () {
  ...
  async function deployXMBLTokenFixture() {
    ...
    await xmblToken.grantRole(MINTER_ROLE, minter.address);
  ...
  describe("Deployment and Initialization", function () {
    it("Should deploy with correct name, symbol, and ERC-6551 configuration", async function () {
      ...
      expect(await xmblToken.erc6551Registry()).to.equal(registry.address);
      expect(await xmblToken.tbaImplementation()).to.equal(tbaImplementation.address);
    });
  ...
  describe("NFT Minting with Token Bound Accounts", function () {
    ...
    it("Should create unique TBAs for each NFT", async function () {
      ...
      await xmblToken.connect(minter).mintWithTBA(user1.address, ethers.utils.parseEther("1.0"), ethers.constants.AddressZero);
      await xmblToken.connect(minter).mintWithTBA(user2.address, ethers.utils.parseEther("2.0"), ethers.constants.AddressZero);
    ...
    it("Should only allow minters to mint NFTs", async function () {
      ...
      await expect(
        xmblToken.connect(user1).mintWithTBA(user2.address, ethers.utils.parseEther("1.0"), ethers.constants.AddressZero)
    ...
    it("Should increment token IDs correctly", async function () {
      ...
      await xmblToken.connect(minter).mintWithTBA(user1.address, ethers.utils.parseEther("1.0"), ethers.constants.AddressZero);
      await xmblToken.connect(minter).mintWithTBA(user1.address, ethers.utils.parseEther("2.0"), ethers.constants.AddressZero);
      await xmblToken.connect(minter).mintWithTBA(user1.address, ethers.utils.parseEther("3.0"), ethers.constants.AddressZero);
    ...
    it("Should validate minting parameters", async function () {
      ...
      // Zero deposit value
      await expect(
        xmblToken.connect(minter).mintWithTBA(user1.address, 0, ethers.constants.AddressZero)
  ...
  describe("Token Bound Account Operations", function () {
    it("Should calculate correct TBA address using ERC-6551 standard", async function () {
      ...
      const expectedTBA = await registry.account(tbaImplementation.address, salt, chainId, xmblToken.address, tokenId);
  ...
  describe("Enumeration and Batch Operations", function () {
    it("Should track user's NFT collection", async function () {
      ...
      await xmblToken.connect(minter).mintWithTBA(user1.address, ethers.utils.parseEther("1.0"), ethers.constants.AddressZero);
      await xmblToken.connect(minter).mintWithTBA(user1.address, ethers.utils.parseEther("2.0"), ethers.constants.AddressZero);
      await xmblToken.connect(minter).mintWithTBA(user2.address, ethers.utils.parseEther("3.0"), ethers.constants.AddressZero);
  ...
  describe("Access Control", function () {
    it("Should manage minter roles correctly", async function () {
      ...
      await xmblToken.connect(user1).mintWithTBA(user1.address, ethers.utils.parseEther("1.0"), ethers.constants.AddressZero);
    ...
    it("Should revoke minter roles", async function () {
      ...
      await xmblToken.connect(owner).revokeRole(MINTER_ROLE, minter.address);
      
      expect(await xmblToken.hasRole(MINTER_ROLE, minter.address)).to.be.false;
      
      await expect(
        xmblToken.connect(minter).mintWithTBA(user1.address, ethers.utils.parseEther("1.0"), ethers.constants.AddressZero)
  ...
  describe("Integration with DeFi Protocols", function () {
    it("Should allow TBA to interact with external contracts", async function () {
      ...
      await xmblToken.connect(user1).executeTBACall(1, mockDefi.address, ethers.utils.parseEther("0.1"), stakeData);
  ...
  describe("Security and Edge Cases", function () {
    ...
    it("Should handle contract upgrades gracefully", async function () {
      ...
      await xmblToken.connect(owner).updateTBAImplementation(newTbaImplementation.address);
      
      expect(await xmblToken.tbaImplementation()).to.equal(newTbaImplementation.address);
    ...
    it("Should validate all contract interactions", async function () {
      ...
      await expect(
        xmblToken.connect(user1).executeTBACall(1, user1.address, ethers.utils.parseEther("10.0"), "0x")
    ...
    it("Should handle extreme values correctly", async function () {
      ...
      await xmblToken.connect(minter).mintWithTBA(user1.address, maxValue, ethers.constants.AddressZero);
