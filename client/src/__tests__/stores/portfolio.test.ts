...
vi.mock('../../services/apiService', () => ({
  default: {
    ...
    getYieldData: vi.fn(async () => ({ currentAPY: 5 })),
    getNFTBalances: vi.fn(async (tokenIds) => tokenIds.map(id => ({ tokenId: id, balance: '200', tbaAddress: `0x${id}...` }))),
    claimYields: vi.fn(async (tokenId) => `0xtx${tokenId}...`),
    getTBABalance: vi.fn(async () => '300')
  }
}))
...
vi.mock('../../services/web3Service', () => ({
  web3Service: {
    getTBABalance: vi.fn(async () => '300'),
    claimYields: vi.fn(async (tokenId) => `0xtx${tokenId}...`),
    batchClaimYields: vi.fn(async (tokenIds) => tokenIds.map(id => `0xtx${id}...`))
  }
}))
