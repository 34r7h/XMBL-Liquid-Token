...
class ApiService {
  ...
  private async makeRequest<T>(
    ...
    // Handle request deduplication (skip for retry requests to avoid deadlock)
    const requestKey = `${method}:${url}:${JSON.stringify(data || {})}`
    if (!options.isRetry && this._pendingRequests.has(requestKey)) {
      return this._pendingRequests.get(requestKey)!
    }

    // Create the request promise
    const requestPromise = this._executeRequest<T>(method, url, options, data, startTime, cacheKey, requestKey)
    
    // Store it for deduplication (skip for retry requests)
    if (!options.isRetry) {
    ...
    try {
      ...
      // Clean up the pending request (skip for retry requests)
      if (!options.isRetry) {
  ...
  private async _executeRequest<T>(
    ...
    const concurrencyKey = `${method}:${url}`
    if (this._activeRequests.size >= this.maxConcurrentRequests) {
      await new Promise((resolve, reject) => {
        this._queuedRequests.push({ key: concurrencyKey, resolve, reject })
      })
    }

    this._activeRequests.add(concurrencyKey)
    ...
    try {
      ...
      while (true) {
        try {
          ...
          // Don't retry client errors (4xx) or if retry is disabled or max retries exceeded
          if (!options.retry || retries > maxRetries || 
              (error.name === 'AbortError')) {
            throw error
          }

          // Wait before retrying - much shorter delay for tests
          const isTest = typeof (globalThis as any).vi !== 'undefined' || process.env.NODE_ENV === 'test'
          const delay = isTest ? 10 : 1000 * retries  // 10ms in tests vs exponential backoff
      ...
      if (!response!.ok) {
        ...
        try {
          ...
          // Use statusText if available, otherwise generic error
        ...
        if (response!.status === 401 && this.hasRefreshToken() && !options.isRetry) {
          try {
            const newToken = await this.refreshAuthToken()
            this.setAuthToken(newToken)
            
            // Retry original request with new token - mark as retry to prevent infinite recursion
            return this.makeRequest<T>(method, url, { ...options, isRetry: true }, data)
          } catch (refreshError) {
            this.clearAuthToken()
        ...
        // Create error with status and data properties
        const error = new Error(`HTTP ${response!.status}: ${errorText}`) as any
        error.status = response!.status
        try {
          // Try to parse error response as JSON for data property
      ...
      // For mocked responses in tests, try json() first if available
      try {
        if (response!.json && typeof response!.json === 'function') {
        ...
        // If parsing fails, return the raw response for tests
        responseData = response!
      }
      ...
      this._activeRequests.delete(concurrencyKey)
  ...
  get pendingRequests(): Set<string> {
    return this._activeRequests
  }

  // Clear interceptors for testing
