...
class ApiService {
    ...
    private requestInterceptors: ((options: RequestInit) => RequestInit)[] = []
    ...
    addRequestInterceptor(interceptor: (options: RequestInit) => RequestInit): void {
    ...
    private async makeRequest<T>(
        ...
        if (this._activeRequests.size >= this.maxConcurrentRequests) {
            await new Promise((resolve) => {
                this._queuedRequests.push({ key: requestKey, resolve, reject: resolve })
            })
        }
        ...
        try {
            ...
            while (retries <= maxRetries) {
                try {
                    ...
                    if (modifiedOptions.signal) {
                        ...
                        const originalSignal = modifiedOptions.signal
                    ...
                    if (modifiedResponse.status === 401 && this.hasRefreshToken()) {
                        try {
            ...
            // Handle different error types for test compatibility
            if (error.name === 'AbortError') {
                throw new Error('AbortError')
            }
        ...
        if (options.onProgress) {
            return new Promise((resolve, reject) => {
                try {
                    ...
                    // Check if addEventListener is available (for test environment)
